...
def get_items(
    ...
    def _get_items(pos_profile, price_list, item_group, search_value, customer=None):
        ...
        if items_data:
            item_prices = get_items_prices_dict(
                items_data, price_list, pos_profile, customer
            )
...
def get_items_details(pos_profile, items_data, price_list=None, customer=None):
    ...
    @redis_cache(ttl=ttl or 1800)
    def __get_items_details(pos_profile, items_data, price_list=None, customer=None):
        return _get_items_details(pos_profile, items_data, price_list, customer)

    def _get_items_details(pos_profile, items_data, price_list=None, customer=None):
        today = nowdate()
        pos_profile = json.loads(pos_profile)
        ...
        if len(items_data) > 0:
            for item in items_data:
                ...
                if item_prices.get(item_code):
                    item_price = (
                        item_prices.get(item_code).get(item.get("stock_uom"))
                        or item_prices.get(item_code).get("None")
                        or {}
                    )
    ...
    if _pos_profile.get("posa_use_server_cache"):
        return __get_items_details(pos_profile, items_data, price_list, customer)
    else:
        return _get_items_details(pos_profile, items_data, price_list, customer)
...
def create_customer(
    ...
    if method == "create":
        ...
        if mobile_no:
            ...
            not mobile_no
            and customer_doc.mobile_no
            and customer_doc.mobile_no not in ["", None, "None"]
        ):
            set_customer_info(customer_doc.name, "mobile_no", 0)
        ...
        if email_id:
            ...
            not email_id
            and customer_doc.email_id
            and customer_doc.email_id not in ["", None, "None"]
        ):
            set_customer_info(customer_doc.name, "email_id", "")
        return customer_doc
...
def get_items_prices_dict(items_data, price_list, pos_profile, customer):
    today = nowdate()
    items_list = []
    for item in items_data:
        if item.get("item_code"):
            items_list.append(item.get("item_code"))
        else:
            frappe.throw(_(f"Item Code is missing in {item}"))
    item_prices_data = frappe.get_all(
        "Item Price",
    ...
    # convert item_prices_data to dict
    item_prices_dict = {}
    for item_price in item_prices_data:
        item_prices_dict[item_price["item_code"]] = item_price

    item_prices = {}
    for item_code in items_list:
        item_prices.setdefault(item_code, {})
        item_data = item_prices_dict.get(item_code)
        if item_data:
            item_prices[item_code][item_data.get("uom") or "None"] = item_data
        else:
            item_prices[item_code]["None"] = {
                "price_list_rate": 0,
                "currency": pos_profile.get("currency"),
                "item_code": item_code,
                "uom": "None",
